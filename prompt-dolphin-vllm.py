...
pipe = pipeline("text-generation", model="cognitivecomputations/dolphin-2.0-mistral-7b", torch_dtype=torch.bfloat16, device=0)
...
with open('all_prompts_train_with_gpt-4-1106-preview_responses_explicit_refusal.jsonl') as f_in:
    with open('f_out-dolphin-vllm.jsonl', 'w') as f_out:
        with open('f_out-dolphin-dangerous.jsonl', 'w') as f_out_dangerous:
            with open('f_out-dolphin-offensive.jsonl', 'w') as f_out_offensive:
                i = 0
                ...
                llm = LLM(model="cognitivecomputations/dolphin-2.0-mistral-7b")
                ...
                for data, output in zip(datas, outputs):
                    ...
                    out_data = {
                        ...
                        "rejected_model": "cognitivecomputations/dolphin-2.0-mistral-7b", # where applicable
